...
$cantidad;
  ...
  if(isset($_GET['id_usuario']) and isset($_GET['id_prod']) and isset($_GET['cantidad'])) {
      ...
      if ($cantidad == 0) {
        $consulta = "INSERT INTO pedidos (id_usuario_fk, id_producto_fk, cantidad) VALUES ('$id_usuario', '$id_producto', 1)"; 
      } else {
        $cantidad +=1;
        $consulta = "UPDATE pedidos SET cantidad='$cantidad' WHERE id_usuario_fk = '$id_usuario' AND id_producto_fk = '$id_producto'";
      }
      $resultado = mysqli_query($con, $consulta);
      header("Location: ../paginas/pedir.php?mas=ok");
  }

//header("Location: ../paginas/pedir.php?mas=error");
